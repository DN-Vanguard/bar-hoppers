...
import { cocktailParty } from "../../utils/API";
import Avatar from '@mui/material/Avatar';

export default function Party({ currentPage, handlePageChange }) {
    const [partyDrinks, setPartyDrinks] = useState([]);

    useEffect(() => {
        const timeToParty = async (event) => {

            try {
                const response = await cocktailParty();
                if (!response.ok) {
    ...
    const renderPartyDrinks = () => {
        return (
            partyDrinks.map((drink) => {
                return (
    ...
    return (
        <div>
            <div className="SuggestedPageUI">
                <h3 className="Header-SuggestedDrink">Time to Party!</h3>
                <div className="AllSuggestedDrinks">
                    {partyDrinks ? renderPartyDrinks() : <h3 className="NoResult">Party is not yet ready!</h3>}
                </div>
            </div>
